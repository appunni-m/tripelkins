...
export function developmentPlan(w) {
  const goal = w.memory.goals.find(g=>g.status === "active"), care = careContext(w);
  const density = densitySummary(w), n = w.creatures.length;
  const growing = !goal || ["grow","care"].includes(goal.kind);
  ...
  if (project) children.push({ id:"project", kind:project.type,
    ...
    status:project.blocked?"blocked":"working", remaining:Math.max(0,project.required-project.progress) });
  const title=goal ? ({grow:`Grow to ${goal.target.toLocaleString()} Tripelkins`,care:"Keep everyone comfortable",
    wood:`Store ${goal.target} wood`,ore:`Store ${goal.target} ore`,blocks:`Save ${goal.target} blocks`,bridge:"Finish the river crossing"}[goal.kind]) : "Grow a healthy, spacious colony";
  return { parent:goal?.id||"colony", title,
