...
export function createCommunityUI({ engine, getWorld, ready, modal, closeModal, save, openIntelligence, onConsent, listen }) {
  ...
  function update(status, notify = false) {
    ...
    if ($("thought-log").open) {
      ...
      const selected=review?.choices?.find(choice=>choice.id===review.selected);
      const allocation=selected?.allocation ? Object.entries(selected.allocation)
        .filter(([,count])=>count>0).map(([task,count])=>`${count} ${names[task]||task}`).join(", ") : "";
      const groupSummary=review ? `Last group choice: ${review.selected || "thinking"}${allocation ? ` (${allocation})` : ""} from ${review.candidates} feasible schedules.${review.source ? ` Source: ${review.source}.` : ""}${review.outcome ? ` ${review.outcome}` : ""}` : "";
      const development=brainStatus.developmentReview;
      const buildingSummary=development ? ` Latest building review: ${development.selectedLabel || `${development.candidates.length} listed choices`}. ${development.status}` : "";
      const thinking=review || development ? `${brainStatus.scheduleCalls} group model requests · ${brainStatus.developmentCalls} building model requests this session. ${review?.reason ? `${review.reason} ` : ""}${groupSummary}${buildingSummary}` : "";
